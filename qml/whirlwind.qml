...
Rectangle {
	...
	property var lastrotateangle: 0;
	...
	// Whirlwind Demo - Timer to change width of video surface
	Timer {
		interval: 2000; running: completed ? true : false; repeat: true
		onTriggered: {
			if (vlcPlayer.state == 3) {
				if (vlcSurface.width < vlcSurface.parent.width) {
					vlcSurface.width = vlcSurface.parent.width;
					vlcSurface.height = vlcSurface.parent.height;
				} else {
					vlcSurface.width = vlcSurface.parent.width /2;
					vlcSurface.height = vlcSurface.parent.height /2;
				}
				
			}
		}
	}
	// End Whirlwind Demo - Timer to change width of video surface
    ...
    VlcVideoSurface {
		...
		// Whirlwind Demo - Effect
		anchors.centerIn: parent;
		RotationAnimation on rotation {
			running: vlcPlayer.state == 3 ? true : false
