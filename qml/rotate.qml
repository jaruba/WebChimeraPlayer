...
Rectangle {
	...
	// 3D Rotate Demo - Timer to change 3D angle (modify interval to change speed)
	Timer {
		interval: 20; running: completed ? true : false; repeat: true
		onTriggered: {
			if (vlcPlayer.state == 3) rotateangle = rotateangle +1;
		}
	}
	// End 3D Rotate Demo - Timer to change 3D angle
	
	// Declare Video Layer
    VlcVideoSurface {
        source: vlcPlayer;
		// 3D Rotate Demo - Effect
		...
		transform: Rotation { id: rotatesurface; origin.x: 30; origin.y: 30; axis { x: 0; y: 1; z: 0 } angle: rotateangle }
		smooth: true
		// End 3D Rotate Demo - Effect
    }
	// End Video Layer
